 ...
 */
public class LifeRobotListAdapter extends BaseAdapter {
    private Context context;
    private List<LifeRobotBean> list;

    public LifeRobotListAdapter(Context context, List<LifeRobotBean> list) {
    ...
    public int getItemViewType(int position) {
        int ret = 0;
        LifeRobotBean lifeRobotBean = list.get(position);
        if (lifeRobotBean.isReceived()) {
            ret = 0;
        }else {
            ret = 1;
        }
    ...
    private View getLeftView(int position, View convertView, ViewGroup parent){
        ...
        if (convertView != null){
            ...
            ret = LayoutInflater.from(context).inflate(R.layout.life_robot_left_item,parent,false);
        }
    ...
    private View getRightView(int position, View convertView, ViewGroup parent){
        ...
        if (convertView != null){
            ...
            ret = LayoutInflater.from(context).inflate(R.layout.life_robot_right_item,parent,false);
        }
        ViewHolder holder = (ViewHolder) ret.getTag();
        if (holder == null){
            holder = new ViewHolder(ret);
    ...
    private static class ViewHolder{
        ...
        public void bindView(int position,LifeRobotBean lifeRobotBean){
            name.setText(lifeRobotBean.getUserName());
            text.setText(lifeRobotBean.getText());
        }
    }
}
